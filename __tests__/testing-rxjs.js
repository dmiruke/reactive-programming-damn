...
it('Should map multiple values', () => {
  ...
  let source = scheduler.createColdObservable('--1--2--3--|');
  let expected = '--a--b--c--|';
  //ACT
  let result = source.
    map((value) => value * 10);
  //ASSERT
  scheduler.assertDeepEqual = (actual, expected) => expect(actual).toEqual(expected);
  scheduler.expectObservable(result).toBe(expected, { 'a': 10, 'b': 20, 'c': 30 });
  scheduler.flush();
});
...
it('Should delay all element by the specified time', () => {
  //ARRANGE
  let scheduler = new Rx.TestScheduler();
  let source = scheduler.createColdObservable('-a--------b------c----|');
  //ACT
  let expected = '------a--------b------(c|)';
